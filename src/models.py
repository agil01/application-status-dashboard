...
class HealthCheck(Base):
    ...
    status = Column(String(20), nullable=False)  # operational, degraded, outage, unknown
...
class Incident(Base):
    ...
    __table_args__ = (
        Index("idx_service_active", "service_name", "resolved_at"),
    )
...
class SystemHeartbeat(Base):
    ...
    def __repr__(self):
        return f"<SystemHeartbeat(last_check={self.last_check_at}, status={self.status})>"
...
class Alert(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    alert_type = Column(String(20), nullable=False)  # outage, recovery, heartbeat, system_down
